...
2. Run `supabase/migrations/202607250001_hosted_beta.sql` for a new project,
   then run `supabase/migrations/202608030001_private_canary.sql`. Existing beta
   projects need only the second migration.
...
policies. The private-canary migration adds an owner-scoped delete policy; it
does not change the table shape or existing encrypted rows.
...
Private Canary v0.5 is appropriate for invitation-only product learning. It is
